...
- `bun run test:db-contract`, URL verilmezse PGlite üzerinde geçici ve bellekte çalışan gerçek PostgreSQL açar; Supabase'e özel rol/storage iskeletini kurar, bütün migration'ları sıfırdan uygular ve SQL kontratlarını rollback ile doğrular. Production müşteri verisine dokunmaz.
- `SUPABASE_DB_URL` veya `DATABASE_URL` yalnız ayrı bir doğrulama veritabanını hedeflemek için verilebilir; bu durumda `psql` gerekir. Gerçek production müşteri DB'si kontrat write-test'i için kullanılmaz.
...
- `supabase/migrations/20260716120000_product_readiness_hardening.sql`: upload abuse bütçesi, replay/profile staging cleanup, explicit fresh-project grant'leri, PII-safe owner cleanup ve operasyonel metadata retention.
- `supabase/migrations/20260716123000_validate_product_readiness_constraints.sql`: canlı ön kontrolden sonra welcome note uzunluk constraint'ini mevcut satırlar için de doğrular.
...
- Help mobilde çift adının sağ üstündeki kompakt ikon düğmesidir; masaüstünde sidebar'ın en altında Logout'un hemen üstünde görünür. More içinde tekrar edilmez.
...
- Profil medya önce 5 dakikalık, boyuta bağlı bir `profile-staging` hedefine yüklenir; sunucu boyut ve dosya imzasını doğruladıktan sonra değişmez final profile path'ine taşır. Terk edilen staging objeleri günlük bakımda temizlenir.
...
CRON_SECRET=
ENABLE_ARCHIVE_DOWNLOADS=false
```
...
- `ENABLE_ARCHIVE_DOWNLOADS` launch boyunca `false` veya tanımsız kalır. Yalnız archive runner, ücretli Container altyapısı ve ilgili secret'lar hazır olduğunda `true` yapılır; route bu flag olmadan DB'de archive job oluşturmaz.
...
- Signed PUT imzaları beklenen byte sayısını da bağlar; completion dosyanın magic byte/container imzasını doğrular ve indirme adını doğrulanmış MIME uzantısından üretir.
