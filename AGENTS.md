...
- `GET/POST /api/archives/current`: Private Storage için mevcut ZIP durumu/kaynak özeti döner veya tekil 24 saatlik arşiv hazırlığını başlatır.
- `GET /api/archives/[id]/download`: yalnız kendi customer session'ı için hazır ZIP'i kısa ömürlü signed R2 indirmesine yönlendirir.
- `/api/internal/archives/[id]/*`: yalnız Cloudflare archive runner'ın attempt'e özel HMAC imzalı claim, manifest, ilerleme, tamamlanma ve hata callback'leridir; müşteriye açık değildir.
...
- `archive_jobs`, `archive_job_items`: Download all memories için değişmez medya snapshot'ı, ilerleme ve 24 saatlik ZIP yaşam döngüsü.
...
- Download all memories çıktısı `archives/` altında attempt'e özel geçici anahtarda üretilen system storage'dır ve customer kotasına girmez. Cloudflare Container özel R2 medyalarını sıkıştırmadan doğrudan ZIP'e stream eder; Vercel büyük dosyayı buffer'lamaz. Her çalışma attempt kimliği ve yenilenen iki saatlik lease ile ayrılır; eski attempt callback'leri reddedilir ve aynı ZIP adresini ezemez. Hazır ZIP yalnız session kontrollü URL ile tam bitiş zamanına kadar indirilebilir ve günlük cron tarafından silinir.
...
- Download all memories; fotoğraf, video, voice note ve `messages.csv` içeren tek ZIP hazırlar. Aynı çift için queued/running/unexpired-ready durumunda ikinci iş oluşturulmaz; item sayısı, kaynak boyutu ve ilerleme gösterilir. Demo yalnız kartı gösterir, API isteği başlatmaz.
...
Vercel tarafında gerekli değerler Production, Preview ve Development ortamlarına bilinçli olarak dağıtılmalıdır. `SUPABASE_SERVICE_ROLE_KEY`, R2 secret, `AUTH_*`, `OWNER_SETUP_SECRET`, `TURNSTILE_SECRET_KEY`, `CRON_SECRET` ve `ARCHIVE_*` kesinlikle frontend'e açılmamalı; `NEXT_PUBLIC_` prefix'i almamalıdır.

- `AUTH_PASSWORD_PEPPER`, `AUTH_RATE_LIMIT_SECRET`, `OWNER_SETUP_SECRET`, `CRON_SECRET`, `ARCHIVE_DISPATCH_SECRET` ve `ARCHIVE_CALLBACK_SECRET` en az 32 byte olmalıdır.
- `ARCHIVE_APP_ORIGIN` path içermeyen sabit HTTPS production originidir; callback adresi tarayıcı isteğinden türetilmez.
- Vercel job'a özel callback anahtarını `ARCHIVE_CALLBACK_SECRET` kökünden türetir. Cloudflare Worker kök callback secretını almaz; yalnız imzalı dispatch içindeki tek-job anahtarını Container'a geçirir.
...
- Günlük cron 24 saati geçen reservation'ları kapatır, yarım R2 uploadları ve süresi dolmuş archive ZIP'lerini temizler, owner onaylı deletion job'larını işler ve Supabase/R2 sağlığını owner paneline yazar.
